...
procedure Evaluer_Alea is
    ...
    procedure Calculer_Statistiques
       ...
       Post =>
        ...
        Min + Max <= Taille and Moyenne = Float (Taille) / Float (Borne) and
        Float (Min) <= Moyenne and Moyenne <= Float (Max)
    is
        package Mon_Alea is new Alea (1, Borne);
        use Mon_Alea;
        Frequence_Moyenne_Theorique : Float;
        Valeurs                     : array (1 .. Taille) of Integer;
        Frequences                  : array (1 .. Borne) of Integer;
        Somme_Frequences            : Float;
        Tirage                      : Integer;
    begin
        --  Frequences := ((others => 0), Borne);
        Frequence_Moyenne_Theorique := Float (Taille) / Float (Borne);
        for i in 1 .. Taille loop
            Get_Random_Number(Tirage);
            Valeurs (i)              := Tirage;
            Frequences (Valeurs (i)) := Frequences (Valeurs (i)) + 1;
        end loop;
        Min              := Frequences (1);
        Max              := Frequences (1);
        Somme_Frequences := 0.0;
        for i in 1 .. Borne loop
            ...
            Somme_Frequences := Somme_Frequences + Float (Frequences (i));
        end loop;
        Moyenne := Somme_Frequences / Float (Borne);
