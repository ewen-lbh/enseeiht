...
procedure Exemples_Memoire_Dynamique is
	...
	procedure Illustrer_Memoire_Dynamique_Sans_Free is
		Ptr1 : T_Pointeur_Integer;
		procedure Free is new Ada.Unchecked_Deallocation(Object => Integer, Name => T_Pointeur_Integer);
	begin
		Ptr1 := new Integer;
		Ptr1.all := 5;
		Put_Line ("Ptr1.all = " & Integer'Image (Ptr1.all));
	...
	begin
		...
		-- XXX Quelle est la valeur de Ptr2.all ? on sait plus, ça a été free
		...
		-- Non, il pointe sur de la mémoire qui a été free (use after free)

		-- XXX Que se passe-t-il si on exécute le programme avec valkyrie ?
		-- On remarque qu'il y a un memory leak parce qu'on a pas free Ptr1
	...
	begin
		...
		-- Oui, parce que ce qui est free'd tout seul, c'est pile, mais pas ses éléments.
