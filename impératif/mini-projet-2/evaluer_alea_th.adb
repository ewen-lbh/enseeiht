...
procedure Evaluer_Alea_TH is
	...
	is
		...
		package Mon_TH is
			new TH(T_Cle => Integer, T_Donnee => Integer, Borne => Borne, Hash => id);
		use Mon_TH;
	...
	begin
		...
		for i in 1..Taille loop
			Mon_Alea.Get_Random_Number(nombre);
			if Cle_Presente(Frequences, nombre) then
				Enregistrer(Frequences, nombre, La_Donnee(Frequences, nombre) + 1);
			else
				Enregistrer(Frequences, nombre, 1);
			end if;
		end loop;

		for i in 1..Borne loop
			frequence := 0;
			if Cle_Presente(Frequences, i) then
				frequence := La_Donnee(Frequences, i);
			end if;
