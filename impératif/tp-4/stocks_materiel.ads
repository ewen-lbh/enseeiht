...
package Stocks_Materiel is
    ...
    procedure Enregistrer
       (Stock  : in out T_Stock; Numero_Serie : in Integer;
        Nature : in     T_Nature; Annee_Achat : in Integer) with
    ...
    procedure Nb_Materiels_HS (Stock : in T_Stock) with
       Pre  => Nb_Materiels (Stock) < CAPACITE,
       Post => Nb_Materiels_HS (Stock) < CAPACITE;
    ...
    procedure Supprimer(Stock: in T_Stock; Numero_Serie: in Integer) with
        Post => Nb_Materiels(Stock'New) = Nb_Materiels(Stock'Old) + 1;
