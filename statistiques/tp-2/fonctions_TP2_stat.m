...
function [tirages_angles, tirages_G] = tirages_aleatoires_uniformes(n_tirages, taille)
    tirages_angles = -taille + rand(n_tirages, 1) .* (taille);
...
function [probas_classe_1,probas_classe_2] = probabilites_classe(x_donnees_bruitees,y_donnees_bruitees,sigma,...
    ...
    [~, ~, xs, ys] = centrage_des_donnees(x_donnees_bruitees, y_donnees_bruitees);
    residus_1 = ys - (a_1 * xs + repmat(b_1, 1, length(xs)));
    residus_2 = ys - (a_2 * xs + repmat(b_2, 1, length(xs)));
