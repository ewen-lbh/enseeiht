...
class Entree {
    ...
    /**
     * Construire une entrée à partir d'un intitulé, d'une commande et d'un
     * raccourci
     * 
     * @param unIntitule  l'intitulé de l'entrée
     * @param uneCommande l'intitulé de la commande
     * @param raccourci   le raccourci associé
     */
    // @ requires uneCommande != null; // la commande existe
    // @ requires unIntitule != null && unIntitule.length() > 0;
    // @ // l'intitulé existe
    // @ requires raccourci != 0;
    // @ ensures getIntitule() == unIntitule;
    // @ ensures getCommande() == uneCommande;
    // @ ensures getRaccourci() == raccourci;
    public Entree(String unIntitule, Commande uneCommande, char raccourci) {
    ...
    public /* @ pure @ */ void afficher(int numero) {
        if (commande.estExecutable()) {
            String num = String.format(raccourci == '\0' ? "%d" : "%c", raccourci == '\0' ? numero : raccourci);
            if (num.length() < 2) {
                System.out.print(" ");
            }
