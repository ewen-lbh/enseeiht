...
public class ExempleUnmodifiableList {
  ...
  public static void consulter(List<Integer> nombres, int entier) {
    ...
    System.out.println(
      entier + " dans nombres ? " + (est_present ? "oui" : "non")
    );
    ...
    System.out.println(
      "fréquence de " + entier + " dans nombres ? " + frequence
    );
    ...
    Object entierASuppr = entier;
    nombres.remove(entierASuppr); // sinon ça supprimer à l'indice, et .removeIf(n -> n == entier) supprime toutes les occurences, pas juste la première
    System.out.println("nombres = " + nombres);
    ...
    System.out.println(
      entier + " dans nombres ? " + (encore_present ? "oui" : "non")
    );
    assert frequence >
    1 ==
    encore_present : "frequence et/ou encore_present mal définis !"; // frequence > 1 <=> encore_present

    // Afficher la fréquence de entier dans nombres
    int nouvelle_frequence = Collections.frequency(nombres, entier); // fréquence de entier dans nombres ?

    System.out.println(
      "fréquence de " + entier + " dans nombres ? " + nouvelle_frequence
    );

    assert nouvelle_frequence >= 0 : "Il faut remplacer le XXX_i :";

    // Faire quelques vérifications
    assert frequence - 1 <= nouvelle_frequence &&
    nouvelle_frequence <=
    frequence : "Les fréquences semblent avoir été mal calculées :";

    assert !est_present ||
    nouvelle_taille < taille : "Élément non supprimé : TODO() non remplacé ?";

    assert nouvelle_frequence != frequence ||
    !est_present : "Est-ce que est_present a été calculé correctement ?"; // nouvelle_frequence == frequence => ! est_present

    assert frequence != 1 || !encore_present : "" + // frequence == 1 ==> ! encore_present
    NL +
    TAB +
    ...
    TAB +
    TAB +
    "- Quel est l'indice de l'élémet supprimé ?";
  }
  ...
  public static void exemple2() {
    ...
    List<Integer> mesNombresReadonly = Collections.unmodifiableList(mesNombres);

    for (int i : new int[] { 4, 0, 2, 7 }) {
      try {
        consulter(mesNombresReadonly, i);
        throw new AssertionError("La liste est modifiable !");
      } catch (Exception e) {
        System.out.println("EXCEPTION : " + e);
        System.out.println("mesNombres = " + mesNombresReadonly + NL);
      }
    }
    ...
    try {
      mesNombresReadonly.add(0);
      mesNombresReadonly.set(0, 2);
      mesNombresReadonly.remove(0);
      throw new AssertionError("La liste est modifiable !");
    } catch (Exception e) {
      System.out.println("EXCEPTION : " + e);
      System.out.println("mesNombres = " + mesNombresReadonly + NL);
  ...
  private static final String XXX_s = "il faut remplacer les XXX* !";
  private static final int XXX_i = -1;
  private static final boolean XXX_b = false;
