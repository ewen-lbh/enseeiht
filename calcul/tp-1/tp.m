...
for i = 1:k
  ...
  % Perte d'orthogonalite avec algorithme classique (deux passes)
  Qcgs2 = cgs(cgs(A));
  po(3, i) = norm(eye(n)-Qcgs2'*Qcgs2);

  % Perte d'orthogonalite avec algorithme modifie (deux passes)
  Qmgs2 = mgs(Qmgs);
...
figure('Position',[0.1*L,0.1*H,0.8*L,0.75*H])
    ...
    loglog(x, po(3, :), 'MarkerFaceColor', '#d95319', 'LineWidth',2)
    loglog(x, po(4, :), 'MarkerFaceColor', '#0072bd', 'LineWidth',2)
    grid on
    leg = legend('Gram-Schmidt classique',...
                 'Gram-Schmidt modifie',...
                 'Gram-Schmidt classique, 2 passes',...
                 'Gram-Schmidt modifie, 2 passes',...
                 'Location','NorthWest');
    set(leg,'FontSize',14);
    xlim([x(1) x(end)])
