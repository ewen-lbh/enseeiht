...
% Affichage du maximum de vraisemblance :size(vraisemblances_1)
...
function vraisemblance = vraisemblance(caract_1, caract_2, mu, Sigma)
    x = [caract_1 caract_2];
    vraisemblance = 1 / (2*pi*sqrt(det(Sigma))) * exp(-1/2 * (x - mu) * inv(Sigma) * (x-mu)');
end


function pourcentage_bonne_classification = classif_MV(X_app, mu_1, Sigma_1, mu_2, Sigma_2)
    errors= 0;
    for i = 1:length(X_app)
        caracts_1 = X_app(i, 1, 1);
        caracts_2 = X_app(i, 2, 1);
        vraisemblances_1 = vraisemblance(caracts_1, caracts_2, mu_1, Sigma_1);
        vraisemblances_2 = vraisemblance(caracts_1, caracts_2, mu_2, Sigma_2);

        if vraisemblances_1 < vraisemblances_2
            errors = errors + 1;
        end
    end

    for i = 1:length(X_app)
        caracts_1 = X_app(i, 1, 2);
        caracts_2 = X_app(i, 2, 2);
        vraisemblances_1 = vraisemblance(caracts_1, caracts_2, mu_1, Sigma_1);
        vraisemblances_2 = vraisemblance(caracts_1, caracts_2, mu_2, Sigma_2);
    ...
    pourcentage_bonne_classification = (1 - errors / (2*length(X_app))) * 100;
end
